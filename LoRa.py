...
from SX127x.LORA_V11_config import BOARD
...
def getter(register_address):
    ...
    def decorator(func):
        def wrapper(self):
            return func(self, self.board.xfer([register_address, 0])[1])
        return wrapper
    return decorator
...
def setter(register_address):
    ...
    def decorator(func):
        def wrapper(self, val):
            return self.board.xfer([register_address | 0x80, func(self, val)])[1]
        return wrapper
    return decorator
...
class LoRa(object):
    ...
    board = BOARD()           # init and get the baord's SPI
    ...
    def __init__(self, verbose=True, do_calibration=False, calibration_freq=915):
        ...
        """
        spi = self.board.createSPI()
        print('created SPI:', spi)
        self.board.setup()           #LoRa module initialized and asleep
    ...
    def get_mode(self):
        ...
        :return:    New mode
        """
        self.mode = self.board.xfer([REG.LORA.OP_MODE, 0])[1]
        return self.mode
    ...
    def set_mode(self, mode):
        ...
        self.mode = mode
        return self.board.xfer([REG.LORA.OP_MODE | 0x80, mode])[1]
    ...
    def write_payload(self, payload):
        ...
        return self.board.xfer([REG.LORA.FIFO | 0x80] + payload)[1:]
    ...
    def read_payload(self , nocheck = False):
        ...
        payload = self.board.xfer([REG.LORA.FIFO] + [0] * rx_nb_bytes)[1:]
        return payload
    ...
    def get_freq(self):
        ...
        :rtype:     float
        """
        msb, mid, lsb = self.board.xfer([REG.LORA.FR_MSB, 0, 0, 0])[1:]
    ...
    def set_freq(self, f):
        ...
        i -= mid * 256
        lsb = i
        return self.board.xfer([REG.LORA.FR_MSB | 0x80, msb, mid, lsb])

    def get_pa_config(self, convert_dBm=False):
        v = self.board.xfer([REG.LORA.PA_CONFIG, 0])[1]
        pa_select    = v >> 7
        max_power    = v >> 4 & 0b111
    ...
    def set_pa_config(self, pa_select=None, max_power=None, output_power=None):
        ...
        :return: new register value
        """
        cur = self.get_pa_config()
        pa_select    = cur['pa_select'] if pa_select == None else pa_select
        max_power    = cur['max_power'] if max_power  == None else max_power 
        output_power = cur['output_power'] if output_power == None else output_power
        val = (pa_select << 7) | (max_power << 4) | (output_power)
        return self.board.xfer([REG.LORA.PA_CONFIG | 0x80, val])[1]
    ...
    def get_ocp(self, convert_mA=False):
        v = self.board.xfer([REG.LORA.OCP, 0])[1]
        ocp_on = v >> 5 & 0x01
        ocp_trim = v & 0b11111
        if convert_mA:
            if ocp_trim <= 15:
    ...
    def set_ocp_trim(self, I_mA):
        ...
        ocp_on = self.board.xfer([REG.LORA.OCP, 0])[1] >> 5 & 0x01
        if I_mA <= 120:
            v = int(round((I_mA-45.)/5.))
        else:
            v = int(round((I_mA+30.)/10.))
        v = set_bit(v, 5, ocp_on)
        return self.board.xfer([REG.LORA.OCP | 0x80, v])[1]

    def get_lna(self):
        v = self.board.xfer([REG.LORA.LNA, 0])[1]
        return dict(
                lna_gain     = v >> 5,
    ...
    def set_lna(self, lna_gain=None, lna_boost_lf=None, lna_boost_hf=None):
        ...
        cur = self.get_lna()
        lna_gain = cur['lna_gain'] if lna_gain == None else lna_gain
        lna_boost_lf = cur['lna_boost_lf'] if lna_boost_lf == None else lna_boost_lf
        lna_boost_hf = cur['lna_boost_hf'] if lna_boost_hf == None else lna_boost_hf
        val = (lna_gain << 5) | (lna_boost_lf << 3) | (lna_boost_hf)
        retval = self.board.xfer([REG.LORA.LNA | 0x80, val])[1]
        if lna_gain is not None:
    ...
    def get_fifo_addr_ptr(self):
        return self.board.xfer([REG.LORA.FIFO_ADDR_PTR, 0])[1]

    def set_fifo_addr_ptr(self, ptr):
        return self.board.xfer([REG.LORA.FIFO_ADDR_PTR | 0x80, ptr])[1]

    def get_fifo_tx_base_addr(self):
        return self.board.xfer([REG.LORA.FIFO_TX_BASE_ADDR, 0])[1]

    def set_fifo_tx_base_addr(self, ptr):
        return self.board.xfer([REG.LORA.FIFO_TX_BASE_ADDR | 0x80, ptr])[1]

    def get_fifo_rx_base_addr(self):
        return self.board.xfer([REG.LORA.FIFO_RX_BASE_ADDR, 0])[1]

    def set_fifo_rx_base_addr(self, ptr):
        return self.board.xfer([REG.LORA.FIFO_RX_BASE_ADDR | 0x80, ptr])[1]

    def get_fifo_rx_current_addr(self):
        return self.board.xfer([REG.LORA.FIFO_RX_CURR_ADDR, 0])[1]

    def get_fifo_rx_byte_addr(self):
        return self.board.xfer([REG.LORA.FIFO_RX_BYTE_ADDR, 0])[1]

    def get_irq_flags_mask(self):
        v = self.board.xfer([REG.LORA.IRQ_FLAGS_MASK, 0])[1]
        return dict(
                rx_timeout     = v >> 7 & 0x01,
                rx_done        = v >> 6 & 0x01,
                crc_error      = v >> 5 & 0x01,
    ...
    def set_irq_flags_mask(self,
        ...
        cur = self.get_irq_flags()
        v = self.board.xfer([REG.LORA.IRQ_FLAGS_MASK, 0])[1]
        for i, s in enumerate(cur.keys()):
            this_bit = names[s]
            if this_bit is not None:
                v = set_bit(v, i, this_bit)
        return self.board.xfer([REG.LORA.IRQ_FLAGS_MASK | 0x80, v])[1]

    def get_irq_flags(self):
        v = self.board.xfer([REG.LORA.IRQ_FLAGS, 0])[1]
        return dict(
                rx_timeout     = v >> 7 & 0x01,
                rx_done        = v >> 6 & 0x01,
                crc_error      = v >> 5 & 0x01,
    ...
    def set_irq_flags(self,
        ...
        for i, s in enumerate(cur.keys()):
            ...
            if this_bit is not None:
                v = set_bit(v, i, this_bit)
        return self.board.xfer([REG.LORA.IRQ_FLAGS | 0x80, v])[1]
    ...
    def get_rx_nb_bytes(self):
        return self.board.xfer([REG.LORA.RX_NB_BYTES, 0])[1]

    def get_rx_header_cnt(self):
        msb, lsb = self.board.xfer([REG.LORA.RX_HEADER_CNT_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def get_rx_packet_cnt(self):
        msb, lsb = self.board.xfer([REG.LORA.RX_PACKET_CNT_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def get_modem_status(self):
        status = self.board.xfer([REG.LORA.MODEM_STAT, 0])[1]
        return dict(
    ...
    def get_pkt_snr_value(self):
        v = self.board.xfer([REG.LORA.PKT_SNR_VALUE, 0])[1]
        return float(256-v) / 4.

    def get_pkt_rssi_value(self):
        v = self.board.xfer([REG.LORA.PKT_RSSI_VALUE, 0])[1]
        return v - (164 if self.board.low_band else 157)     # See datasheet 5.5.5. p. 87

    def get_rssi_value(self):
        v = self.board.xfer([REG.LORA.RSSI_VALUE, 0])[1]
        return v - (164 if self.board.low_band else 157)     # See datasheet 5.5.5. p. 87

    def get_hop_channel(self):
        v = self.board.xfer([REG.LORA.HOP_CHANNEL, 0])[1]
        return dict(
                pll_timeout          = v >> 7,
    ...
    def get_modem_config_1(self):
        val = self.board.xfer([REG.LORA.MODEM_CONFIG_1, 0])[1]
        return dict(
                bw = val >> 4 & 0x0F,
    ...
    def set_modem_config_1(self, bw=None, coding_rate=None, implicit_header_mode=None):
        cur = self.get_modem_config_1()
        bw                   = cur['bw'] if bw == None else bw
        coding_rate          = cur['coding_rate'] if coding_rate == None else coding_rate
        implicit_header_mode = cur['implicit_header_mode'] if implicit_header_mode == None else implicit_header_mode
        val = (implicit_header_mode) | (coding_rate << 1) | (bw << 4)
        return self.board.xfer([REG.LORA.MODEM_CONFIG_1 | 0x80, val])[1]
    ...
    def get_modem_config_2(self, include_symb_timout_lsb=False):
        val = self.board.xfer([REG.LORA.MODEM_CONFIG_2, 0])[1]
        d = dict(
    ...
    def set_modem_config_2(self, spreading_factor=None, tx_cont_mode=None, rx_crc=None):
        # RegModemConfig2 contains the SymbTimout MSB bits. We tack them back on when writing this register.
        cur = self.get_modem_config_2(include_symb_timout_lsb=True)
        spreading_factor = cur['spreading_factor'] if spreading_factor == None else spreading_factor
        tx_cont_mode     = cur['tx_cont_mode'] if tx_cont_mode == None else tx_cont_mode
        rx_crc           = cur['rx_crc'] if rx_crc == None else rx_crc
        val = (spreading_factor << 4) | (tx_cont_mode << 3) | (rx_crc << 2) | cur['symb_timout_lsb']
        return self.board.xfer([REG.LORA.MODEM_CONFIG_2 | 0x80, val])[1]
    ...
    def get_modem_config_3(self):
        val = self.board.xfer([REG.LORA.MODEM_CONFIG_3, 0])[1]
        return dict(
    ...
    def set_modem_config_3(self, low_data_rate_optim=None, agc_auto_on=None):
        cur = self.get_modem_config_3()
        low_data_rate_optim = cur['low_data_rate_optim'] if low_data_rate_optim == None else low_data_rate_optim
        agc_auto_on         = cur['agc_auto_on'] if agc_auto_on == None else agc_auto_on
        val = (low_data_rate_optim << 3) | (agc_auto_on << 2)
        return self.board.xfer([REG.LORA.MODEM_CONFIG_3 | 0x80, val])[1]
    ...
    def get_symb_timeout(self):
        ...
        msb, lsb = self.board.xfer([SYMB_TIMEOUT_MSB, 0, 0])[1:]    # the MSB bits are stored in REG.LORA.MODEM_CONFIG_2
        msb = msb & 0b11
        return lsb + 256 * msb

    def set_symb_timeout(self, timeout):
        bkup_reg_modem_config_2 = self.board.xfer([REG.LORA.MODEM_CONFIG_2, 0])[1]
        ...
        old_msb = self.board.xfer([REG.LORA.MODEM_CONFIG_2  | 0x80, reg_modem_config_2])[1] & 0x03
        old_lsb = self.board.xfer([REG.LORA.SYMB_TIMEOUT_LSB | 0x80, lsb])[1]
        return old_lsb + 256 * old_msb

    def get_preamble(self):
        msb, lsb = self.board.xfer([REG.LORA.PREAMBLE_MSB, 0, 0])[1:]
        return lsb + 256 * msb

    def set_preamble(self, preamble):
        msb = preamble >> 8
        lsb = preamble - msb * 256
        old_msb, old_lsb = self.board.xfer([REG.LORA.PREAMBLE_MSB | 0x80, msb, lsb])[1:]
        return old_lsb + 256 * old_msb
    ...
    def get_fei(self):
        msb, mid, lsb = self.board.xfer([REG.LORA.FEI_MSB, 0, 0, 0])[1:]
        msb &= 0x0F
    ...
    def get_register(self, register_address):
        return self.board.xfer([register_address & 0x7F, 0])[1]

    def set_register(self, register_address, val):
        return self.board.xfer([register_address | 0x80, val])[1]

    def get_all_registers(self):
        # read all registers
        reg = [0] + self.board.xfer([1]+[0]*0x3E)[1:]
        self.mode = reg[1]
        return reg
